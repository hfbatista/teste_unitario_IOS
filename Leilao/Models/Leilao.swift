...
class Leilao {
    ...
    fileprivate func ultimoLance(_ lances: [Lance]) -> Lance {
    ...
    func podeDarlance(_ usuario: Usuario, _ listaDeLances: [Lance]) -> Bool {
        return ultimoLance(listaDeLances).usuario != usuario && quantidadeLancesDoUsuario(usuario) < 5
    }
    
    func quantidadeLancesDoUsuario(_ usuario: Usuario) -> Int {
